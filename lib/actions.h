...
void relay_msg(const int clientSock,struct data_wrapper *data);
void store_msg(struct data_wrapper *data);
void client_update(struct data_wrapper *data,int sock, int64_t deadline);
void send_hostname_to_client(struct data_wrapper *data,int sock,char *hostname);
void send_peer_list_to_client(struct data_wrapper *data,int sock);
