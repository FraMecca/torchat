...
static bool
opt_longint (struct argparse *self, const struct argparse_option *opt, int flags)
{
    const char *s = NULL;
    if (self->optvalue) {
...
argparse_long_opt(struct argparse *self, const struct argparse_option *options)
{
    for (; options->type != ARGPARSE_OPT_END; options++) {
        ...
        return argparse_getvalue(self, options, opt_flags | OPT_LONGF);
    }
    return -2;
}

int
argparse_init(struct argparse *self, struct argparse_option *options, const char *const *usages, int flags)
{
...
argparse_usage(struct argparse *self)
{
    ...
    for (; options->type != ARGPARSE_OPT_END; options++) {
        ...
        if (options->type == ARGPARSE_OPT_INTEGER) {
            len += strlen("=<int>");
        } else if (options->type == ARGPARSE_OPT_LONG) {
            len += strlen("=<long>");
